...
pub fn cmd(args: &Args) -> Result<()> {
    ...
    let runs: Vec<Value> = text.lines().filter(|l| l.starts_with('{')).map(serde_json::from_str).collect::<std::result::Result<_, _>>()?;
    ...
    println!("| Slice | Mode | Threads | Runs | Median wall s | First-run wall s | Median CPU s | Peak RSS MiB | MiB/s | Lines/s | Load1 (median) |");
    println!("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |");
    for ((slice, order, threads), rs) in &groups {
        let med = |k: &str| median(&mut rs.iter().map(|r| f(r, k)).collect::<Vec<_>>());
        let first = rs.iter().min_by_key(|r| r["at"].as_i64().unwrap_or(0)).map(|r| f(r, "wall_s")).unwrap_or(f64::NAN);
        println!(
    ...
    for r in &runs {
        if let Some(d) = r["digest"].as_str() {
            ...
            by_slice.entry(r["slice"].as_str().unwrap_or("").into()).or_default().entry(d.into()).or_default().push(key);
        }
    }
    for (slice, digests) in &by_slice {
        if digests.len() == 1 {
            println!("slice {slice}: all parsing variants agree on totals ({} runs)", digests.values().map(Vec::len).sum::<usize>());
        } else {
